...
- **바탕화면 창을 이 창의 소유자(owner)로 지정한다** (`window.rs::own_by_desktop`,
  ...
  - **알려진 대가 — 한/영 전환.** 소유 관계는 활성화도 가로챈다: Windows 는 소유자가 활성화되면
    그 그룹의 **마지막 활성 팝업**으로 활성화를 넘기므로, "바탕화면 보기" 뒤 전경이
    `Progman ↔ Tauri Window` 로 진동하고 포커스 가진 컨트롤이 없어(`focus=0x0`) 한/영 키가
    갈 곳을 잃는다. 작업 표시줄을 한 번 클릭하면 풀린다. **아직 미해결.**
    실패한 해결 시도 둘 (되살리지 말 것):
    `WS_EX_TOOLWINDOW`+`WS_EX_APPWINDOW` 제거는 가로채기를 막지 못했고,
    소유를 "바탕화면 보기" 동안으로만 좁히고 `WS_EX_NOACTIVATE` 를 켜는 판은 탐지가 늦어
    **Win+D 때 위젯이 아예 보이지 않아** 상시 소유보다 명백히 나빴다.
  - git 저장소가 아니라 되돌릴 안전망이 없다. 이 판의 보관본은 `.backup/` 에 있다.
